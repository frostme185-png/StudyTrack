...
- `src/main/main.js` — single file containing everything in the main process: data load/save, the 1s auto-track polling loop, the manual timer's clock, native helper compilation, IPC handlers, tray, sticky widget lifecycle, daily reminder notifications, auto-updater.
...
- `src/renderer/sticky.html` — floating widget: progress ring, mascot emoji, level icon, collapsible badge strip, running-manual-timer display, 6 themes.
- `src/renderer/shared.js` and `src/renderer/achievements.js` — plain `<script src>` includes loaded by **both** renderers, in that order, before their own inline scripts. With no build step this is the only way the two pages share code. `shared.js` holds `dateStr`/`todayStr`/`monthStr`, `fmtClock` and `ACCENT_COLORS`; put anything else both pages need here rather than copying it.
...
- **`streakColor`** picks the 4-step intensity ramp (`--sc1..4`) for the calendar and heatmap; **`accentColor`** independently picks `--accent` for buttons/toggles/tabs. They were one value once, which meant choosing a colour visibly changed the widget but nothing on the dashboard. `ACCENT_COLORS` lives in `shared.js` and is read by both pages; `STREAK_PALETTE` (the 4-step ramps) stays in `index.html`, which is the only page that needs it. The two used to be separate identical tables, one per renderer, so adding a colour to one left the other silently falling back to green.
...
- **Peak focus window** — best contiguous 3-hour block in the `hours` buckets, wrapping past midnight. Its share is measured against *bucketed* time, not all-time, because rows predating the `hours` field carry none and would deflate every percentage. Stays silent below 25% (a flat spread gives 3/24 ≈ 13%, so anything near that is a maximum picked out of noise, not a finding).
- **Habit shift** — biggest month-over-month move in one item's *share* of the month; share rather than raw seconds, so a busier month doesn't read as a change in what you're doing. Needs ≥30 min in both months and a ≥15-point move. Shares are zero-sum within a month, so a riser always has a mirror-image faller tied with it — ties break toward the riser, which is the more useful sentence ("Anki is new and already 66%" over the same fact told backwards).
- **Top item this month** — % share of this month's total time.

**User-facing text must go through `siteLabel()` / `siteIcon()`, never the raw `domain`.** `domain` is a storage key (`app:anki`); Insights used to print that verbatim beside a globe icon while the Time Breakdown right above it showed "Anki". Both helpers fall back sensibly for domains no longer in the tracked list, which still own history.
...
Data + pure computation live in `src/renderer/achievements.js`, loaded by both pages after `shared.js` (it calls `dateStr()` from there) and before their own inline `<script>`.
...
- **Icons**: a complete set ships in `assets/badges/`, produced by `assets/badges/generate.js` — edit that and re-run `node assets/badges/generate.js` rather than hand-editing the SVGs, which it overwrites. Keeping 21 level icons generated is what holds them to one silhouette, one palette and one set of proportions. Design notes (why tier reads from shape+material and sub-rank from pip count, why Diligent is a gem and not a flame) are in `assets/badges/README.md`. Hand-drawn art dropped in under the same filename wins; missing files still fall back to a built-in emoji (`resolveAchievementIcon()`), so nothing breaks if one is deleted.
